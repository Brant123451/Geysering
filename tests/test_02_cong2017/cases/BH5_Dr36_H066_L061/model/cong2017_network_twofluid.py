...
class NetworkCase:
    ...
    # ---- Cong, Chan & Lee (2017) layout extensions (case-snapshot, 2026-07-06) ----
    # The V&W(2011) apparatus has the pocket UPSTREAM (pressurized, both ends
    # closed).  The Cong (2017) Series-B apparatus differs in three ways, each
    # gated on the fields below so the V&W defaults reproduce the old behavior:
    #   * upstream end = CONSTANT-HEAD RESERVOIR (overflow-weir tank), head
    #     reservoir_head [m] above the pipe CROWN (the riser/tank level datum);
    #   * air pocket = the DOWNSTREAM section [L_up+L_mid, L_tunnel] behind the
    #     release ball valve, initially at ATMOSPHERIC pressure (air_head=0);
    #   * the geysering drive is the DYNAMIC compression of that pocket by the
    #     reservoir inflow + reflected wave (measured up to 1.9*H0), so the
    #     driven-regime cap on H_op is configurable (Hop_cap) instead of the
    #     V&W initial-margin cap (air_head - init_water_level).
    reservoir_head: float | None = None   # upstream constant head above pipe crown [m]
    pocket_downstream: bool = False       # pocket behind the valve at x = L_up+L_mid
    ...
    def x_riser(self) -> float:
        if self.x_riser_at is not None:
            return self.x_riser_at
        return self.L_up + self.L_mid          # tower at the coupling after the middle pipe

    @property
    def x_valve(self) -> float:
        # V&W: valve between the upstream pocket and the water column (x=L_up);
        # Cong: ball valve at the upstream edge of the downstream pocket.
        return (self.L_up + self.L_mid) if self.pocket_downstream else self.L_up

    @property
    def x_transducer(self) -> float:
        if self.x_transducer_at is not None:
            return self.x_transducer_at
        return self.L_up + 1.07                # V&W2011: transducer 1.07 m downstream of the valve

    @property
    def V_air(self) -> float:
        if self.pocket_downstream:
            return self.L_down * self.A        # pocket = downstream section behind the valve
...
def run_network(case: NetworkCase, verbose: bool = True) -> dict:
    ...
    iv = int(np.clip(round(case.x_valve / dx - 0.5), 1, Nt - 2))   # release-valve cell
    ...
    # ---- initial state ----
    # V&W(2011): upstream pipe = compressed air pocket; middle+downstream pipes
    #            water-filled; tower to Y_fs0; both far ends closed.
    # Cong(2017) Series B (pocket_downstream): pipe water-filled from the
    #            constant-head tank up to the ball valve; the emptied section
    #            behind the valve [x_valve, L_tunnel] holds the ATMOSPHERIC
    #            pocket (air_head = 0); closed cap at the downstream end.
    Alt = A * np.ones(Nt)
    if case.pocket_downstream:
    ...
    while t < case.t_end - 1e-12:
        ...
        # gauge head of the connected pocket EOS minus the LIQUID WEIGHT standing
        # in the riser.  The resisting load is the water the gas must carry --
        # sum(Al)*dz/Ar, the dense-equivalent column height -- NOT the visible
        # free-surface elevation Yfs: once a gas core occupies part of the bore
        # the column above the nose is all that pushes back (this is exactly the
        # water-slug momentum balance Cong et al. 2017 use, their Eq. (2) with
        # L_w the liquid length above the pocket).  Using Yfs under-read the
        # drive by the core length and stalled the driven climb.
        H_col_liq = float(np.sum(np.clip(Alr, 0.0, Ar)) * dz) / Ar
        H_op = 0.0
        body_t = (Alt / A) < 0.95
        for (i0, i1) in _regions(body_t):
            ...
            H_op = max(H_op, (Preg - P_ATM) / (RHO_L * G) - max(H_col_liq, 0.0))
        # The geyser is powered by the pocket's overpressure margin above the
        # water column.  V&W(2011): the drive is the INITIAL margin (Ha0 - Yfs0);
        # a transient over-compression by the descending column must NOT add
        # spurious drive (that energy belongs to the water and returns to it),
        # so the cap defaults to the initial margin.  Cong(2017) Series B: the
        # pocket STARTS atmospheric and the drive IS the dynamic compression by
        # the reservoir inflow + reflected wave (their measured pocket head
        # reaches ~1.9*H0) -- the physical cap there is supplied by the driver
        # through Hop_cap instead of the (zero) initial margin.
        if case.Hop_cap is not None:
            H_op = min(max(H_op, 0.0), case.Hop_cap)
        else:
        ...
        # DRIVEN-STATE GATE (Cong-2017 snapshot fix): topology (b) (bubbly
        # surface + gassy junction) is the WIDE-tower glug-drain state -- it
        # says gas percolates through a standing column.  While the pocket
        # holds overpressure and is actively expelling the column (H_op > 0,
        # the geysering drive), a bubbly surface does NOT mean the pocket is
        # open to atmosphere: a liquid slug still stands between, and venting
        # the pocket there kills the eruption mid-climb (the model lost the
        # geyser race: pocket blew down at Yint~0.6 m while the experiment
        # compresses to ~1.9*H0 and ejects the column).  In the driven state
        # only the full connected core (topology a) opens the vent.
        driven_now = H_op > 0.02
        breakthrough = bool(junction_gassy and ksurf_pre >= 1
                            and (np.all(alpha_gr_pre[:ksurf_pre] > 0.20)
                                 or (surface_gassy_pre and not driven_now)))
        if breakthrough:
            vent_latched = True
        ...
        if case.reservoir_head is not None:
            # upstream end (x=0): CONSTANT-HEAD RESERVOIR (Cong 2017 overflow
            # tank).  Full-water ghost with zero-gradient through-flow; the head
            # itself enters through the Dirichlet pressure ghost below.
            Alg[0] = A; Qlg[0] = Qlt[0]; cg[0] = ct[0]
        else:
        ...
        if case.reservoir_head is None:
            F1[0] = 0.0                # closed upstream wall passes zero volume
        F1[-1] = 0.0
        ...
        if case.reservoir_head is not None:
            # Dirichlet inlet anchored to the tank head.  IMPORTANT datum note:
            # this model's full-cell pressure field carries P_atm + rho*g*Yfs
            # (the tower-level overlay, crown datum) -- NOT the geometric invert
            # pressure -- so the reservoir ghost must use the SAME convention,
            # P_res = P_atm + rho*g*H0, for the rest state (tank level = riser
            # level = H0) to be an exact discrete equilibrium.  Adding the +D
            # invert offset here would push a permanent rho*g*D spurious inflow.
            P_res = P_ATM + RHO_L * G * case.reservoir_head
            Pth[0] = 2.0 * P_res - Pt[0]
        else:
        ...
        # DIRECTION-AWARE BASE LIMIT (Cong-2017 snapshot fix): the CCFL clamp
        # meters DOWNWARD drainage against rising gas (the V&W vent state); the
        # UPWARD branch is column EXPULSION driven by the compressed pocket --
        # the geyser itself -- and is bounded only by the generous anti-overdraw
        # guard (Cong's measured free-surface speed peaks near 2.8 m/s, far
        # above 3*sqrt(gDr)=1.2 m/s for the 16 mm riser; the old symmetric
        # clamp strangled the eruption into a glug stalemate).
        #
        # GASSY-MOUTH SEAL (Cong-2017 snapshot fix 2): once the pocket occupies
        # the tee, the riser column STANDS ON GAS -- water can only descend
        # through the annular film, i.e. at the CCFL scale, from the moment the
        # junction carries gas (not merely after breakthrough latches).  With
        # the generous 3*sqrt(gDr) leak the entering gas volume was offset by
        # water draining out the base instead of lifting the column: the model
        # sat in a feed/escape stalemate at Yint~0.6 m for ~4 s while the
        # experiment (Fig.9c: La grows monotonically, Lw shrinks only via the
        # film) lifts off within ~1 s of arrival.
        if G1[0] >= 0.0:
            u_base_lim = 10.0 * math.sqrt(G * case.Dr)
        else:
            u_base_lim = (0.5 if (vent_latched or gas_at_junction) else 3.0) \
                * math.sqrt(G * case.Dr)
        q_lim = Ar * u_base_lim
        ...
        for (i0, i1) in _regions(alpha_gt > body_thr):
            ...
            if i0 <= jx < i1:
                if case.pocket_downstream:
                    # Cong(2017): the crown layer SHOOTS PAST the tee toward the
                    # reservoir ("only part of the air pocket enters the riser",
                    # their Fig. 12b) -- the 16 mm mouth is ~10% of the bore, so
                    # the nose keeps advancing upstream while the entry exchange
                    # drinks from the jx cell.  Pinning the nose at the tee (the
                    # V&W closure below) parks the pocket in a steady glug-drain
                    # and the supply line never builds the counter-flow momentum
                    # whose arrest later COMPRESSES the pocket (the 1.9*H0 surge
                    # that powers the geyser).
                    #
                    # MOUTH TOP-UP: the through-layer stays connected across the
                    # tee, so while the nose advances the mouth cell must be
                    # kept AT the body level too -- the riser entry + glug
                    # refill otherwise starve jx below the entry threshold and
                    # the vent seals in a false static state (probe3: 6 s of
                    # frozen equilibrium, Yint decays to zero, no geyser).
                    if alpha_gt[jx] < 0.95 * abar:
                        room_j = max(abar - alpha_gt[jx], 0.0) * A * dx
                        dVj = min(room_j, 0.5 * max(Alt_new[jx], 0.0) * dx, 0.1 * V_reg)
                        if dVj > 0.0 and M_reg > 1.0e-30:
                            dmj = M_reg * dVj / V_reg
                            thin_j = dVj * (void / V_reg) / dx
                            Alt_new[seg] += thin_j
                            Mgt_new[seg] -= dmj * (Mgt_new[seg] / M_reg)
                            Alt_new[jx] -= dVj / dx
                            Mgt_new[jx] += dmj
                            void = np.maximum(A - Alt_new[seg], 0.0) * dx
                            V_reg = float(void.sum())
                            M_reg = float(np.sum(Mgt_new[seg]))
                            alpha_gt[jx] = float(np.clip(1.0 - Alt_new[jx] / A, 0.0, 1.0))
                    sgn = -1
                    tgt = i0 if (i1 - i0 > 1 and alpha_gt[i0] < 0.95 * abar) else i0 - 1
                else:
                    sgn, tgt = +1, jx                    # feed the vent mouth (V&W)
            elif xt[i1 - 1] < case.x_riser and i1 <= jx:
                sgn = +1
                tgt = (i1 - 1) if (i1 - i0 > 1 and alpha_gt[i1 - 1] < 0.95 * abar) else i1
            ...
            if _DISABLE_CROWN:
                u_rel = 0.0
            # FRONT ARREST UNDER ADVERSE HEAD (Cong-2017 snapshot fix): a
            # Benjamin cavity intrudes only while its gas pressure matches the
            # LOCAL ambient crown pressure ahead of the nose to within its own
            # buoyant driving scale (~h_dep).  In stage 1 the sloshing supply
            # line stays quasi-balanced with the pocket, so the front runs at
            # full celerity; once the riser starts venting the pocket, the
            # ambient ahead (fed by the constant-head tank) exceeds the pocket
            # head and the intrusion ARRESTS (Cong Fig. 13a: the front stops
            # at t~9.4 s) -- the reservoir slug then keeps advancing and
            # COMPRESSES the pocket (their measured ~20% volume drop -> the
            # ~1.9*H0 head surge), which is the geysering drive here.  Without
            # the arrest the nose stretches on and buys the pocket free volume,
            # so the compression surge can never build.
            if case.reservoir_head is not None and 0 <= tgt < Nt:
                H_poc_reg = (M_reg * R_GAS * T_GAS / max(V_reg, 1.0e-12)
                             - P_ATM) / (RHO_L * G)
                amb_crown = (float(Pt[tgt]) - P_ATM) / (RHO_L * G) - case.D
                dH_adv = amb_crown - H_poc_reg
                u_rel *= max(1.0 - max(dH_adv, 0.0) / max(h_dep, 1.0e-6), 0.0)
        ...
        if alpha_g_j > case.tower_entry_alpha_min:
            ...
            # CO-CURRENT RELAXATION (Cong-2017 snapshot fix): the Wallis
            # envelope meters COUNTER-current exchange (gas up against water
            # draining down, the V&W narrow-tower state).  When the riser
            # column itself moves UP (geyser expulsion: Cong's Yfs and Yint
            # rise together, v_fs ~ 0.9 m/s), the mouth passes gas co-currently
            # and the envelope rides on the liquid co-flow: j_g <= j_flood + u_l.
            u_mouth_l = float(np.clip(Qlr[0] / max(Alr[0], 1.0e-3 * Ar),
                                      -U_FLUX_MAX, U_FLUX_MAX))
            j_cap = j_flood + max(u_mouth_l, 0.0)
            aperture = min(alpha_g_j / 0.10, 1.0)
            q_gas = min(aperture * Ar * u_in, Ar * j_cap)       # [m^3/s] into the mouth
            # POCKET-WIDE SUPPLY (Cong-2017 snapshot fix): the connected pocket
            # is one gas body at uniform pressure -- gas leaving through the
            # mouth is supplied by the WHOLE pocket, not by the jx cell alone.
            # Drawing on the single cell starved the mouth geometrically
            # (m_up <= 0.5*Mgt[jx] per step with nothing replenishing once the
            # crown current arrested): the feed died ~0.7 s after arrival and
            # the run froze in a false stalemate with the tunnel inventory
            # bit-frozen for 4+ s (budget probe, 2026-07-06).
            reg_jx = None
            for (i0r, i1r) in _regions((Alt_new / A) < 0.95):
                if i0r <= jx < i1r:
                    reg_jx = (i0r, i1r)
                    break
            if reg_jx is not None:
                sl = slice(reg_jx[0], reg_jx[1])
                M_sup = float(np.sum(Mgt_new[sl]))
                m_up = min(rho_g_j * q_gas * dt, 0.2 * max(M_sup, 0.0))
                if M_sup > 1.0e-14:
                    frac = m_up / M_sup
                    Mgt_new[sl] -= Mgt_new[sl] * frac
                    Jgt_new[sl] -= Jgt_new[sl] * frac
            else:
            ...
            if H_op > 1.0e-3:
                ...
                # Fill-to-slug-level front.  V&W's narrow tower carried a slug
                # TRAIN (bubble + comparable liquid slug -> cell-average void
                # ~ 0.5 * core fraction).  Cong's 16 mm riser under sustained
                # reservoir drive carries a CONTINUOUS gas column instead: the
                # Fig. 8 high-speed frames show one coherent core filling the
                # bore with only a thin wall film, and Fig. 9(c) has La growing
                # monotonically with a SOLID water slug Lw = 0.3-0.5 m riding
                # on top.  Feeding at the diluted train level (0.41) built a
                # bubbly 0.25-void column whose top sat AT the packed surface,
                # so the surface escape (flooding-capped, same scale as the
                # mouth feed) balanced the feed exactly -- a stalemate at
                # Yint ~ 0.6 m for 4+ s that the experiment does not show.
                # The coherent-column fill level is the Taylor core fraction
                # itself; the water slug then stays ABOVE the nose, escape
                # stays shut until the nose truly reaches the surface, and
                # hold-up can run away into the eruption.
                alpha_fill = case.tower_core_area_fraction
                k_top = 0
                ...
                # Feed strictly BELOW the surface cell: the pocket feeds the
                # core base-to-nose UNDER the riding water slug; injecting into
                # the surface cell itself handed every new gas parcel straight
                # to the surface-escape flux (both are flooding-capped at the
                # same scale), locking a feed/escape stalemate at Yint ~ 0.6 m
                # for 4+ s.  Kept below the slug, the fed volume lengthens the
                # core and LIFTS the slug (Cong Fig. 9c: La grows monotonically
                # while Lw rides on top); escape only opens when the core
                # itself reaches the surface (breakthrough).
                k_inj = min(k_top, max(ksurf_inj - 1, 0))
            ...
            # closes the junction void vacated by the departing gas.  (A Wallis
            # coupling that BLOCKED this descent when the gas flux ran near its
            # envelope was tried and reverted: it also blocked the annular-film
            # drainage that LIGHTENS the riser column -- Cong Fig.9(c) shows Lw
            # shrinking steadily before the eruption -- and the runaway only
            # started ~2 s later.)
            #
            # (Gating this exchange off for the reservoir-backed layout was
            # tried and reverted, 2026-07-08: it lets the entered gas lift the
            # wide-riser column one-to-one -- Cong Fig. 7a's +0.63 m rise --
            # but it also opens the NARROW pocket's volume during the balanced
            # venting phase, its pressure never rebuilds, and B-H1 loses the
            # geyser.  The exchange stays unconditional; the under-lifted
            # wide-riser column (+0.19 m vs measured +0.63 m) is documented as
            # the known mouth-exchange/film-entrainment closure limitation.)
        ...
        # SURFACE FLOODING CAP (Cong-2017 snapshot fix): for gas to leave through
        # the surface WITHOUT expelling the column, the water above the bursting
        # slug must drain back down around it -- the same counter-current film
        # exchange as at the mouth, bounded by the Wallis envelope.  Uncapped
        # arrival-speed escape let the pocket-fed core vent at ~1 m/s superficial
        # and no hold-up could ever build in the narrow driven riser (steady
        # glug-drain stalemate; the experiment ejects the column instead).  The
        # column-expulsion path (geyser) is unaffected: spill at the rim removes
        # liquid, not this gas flux.
        j_flood_surf = 0.81 * math.sqrt(G * case.Dr * max(RHO_L - rho_atm, 0.0) / RHO_L)
        for kk in (ksurf - 1, ksurf):
            if 0 <= kk < Nr and Mgrs_new[kk] > 1.0e-14:
                ...
                sup_esc = min(alpha_g_esc[kk] * max(u_burst, ug_kk), j_flood_surf)
                esc = min(rho_g_esc[kk] * Ar * sup_esc * dt
