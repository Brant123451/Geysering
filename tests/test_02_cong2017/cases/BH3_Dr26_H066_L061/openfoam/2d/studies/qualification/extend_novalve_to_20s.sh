...
RUN_DIR=/tmp/bh3-2d-qualification/sensitivity_noValve
...
[[ -d "$RUN_DIR/processor0/13" && -d "$RUN_DIR/processor2/13" ]] || {
    echo "Missing complete noValve checkpoint at 13 s" >&2
    exit 3
}
[[ ! -d "$RUN_DIR/processor3" ]] || {
    echo "Expected a three-rank decomposition" >&2
    exit 4
}
...
python3 "$CASE_ROOT/postprocess.py" \
    --run-dir "$RUN_DIR" \
    --output-dir "$RUN_DIR/results20" > postprocess20.stdout.log 2> postprocess20.stderr.log
