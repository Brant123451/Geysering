...
run_dir=/tmp/bh3-2d-qualification/coarse_official_energy
solver=/tmp/bh3-2d-build-official-energy/bin/bh3CompressibleInterFoam
np="${OPENFOAM_NP:-3}"

[[ -x "$solver" ]] || { echo "Missing solver: $solver" >&2; exit 2; }
[[ "$run_dir" == /tmp/bh3-2d-qualification/coarse_official_energy ]] || exit 3
rm -rf -- "$run_dir"
mkdir -p "$run_dir"
