...
int main(int argc, char *argv[])
{
    ...
    const dimensionedScalar isothermalTemperature
    (
    ...
    while (runTime.run())
    {
        ...
        while (pimple.loop())
        {
            ...
            // Deadline-recovery sensitivity: retain pressure-dependent phase
            // densities while replacing the energy equation by an isothermal
            // closure.  This avoids a local negative-temperature failure in
            // the original TEqn without converting the gas to incompressible.
            T = isothermalTemperature;
