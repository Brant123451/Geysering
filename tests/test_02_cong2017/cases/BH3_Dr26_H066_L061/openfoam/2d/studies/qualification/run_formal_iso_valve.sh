...
[[ "$run_dir" == /tmp/bh3-2d-qualification/formal_iso_valve ]] || exit 3
rm -rf -- "$run_dir"
mkdir -p "$run_dir"
cp -a "$case_root/case/." "$run_dir/"

python3 "$case_root/make_blockmesh.py" \
    --config "$case_root/case_config.json" \
...
python3 "$case_root/audit_case.py" \
    --run-dir "$run_dir" \
    --output "$run_dir/paper_audit.json"
