...
run_dir=/tmp/bh3-2d-qualification/medium_iso_valve
solver=/tmp/bh3-2d-build-iso-valve/bin/bh3CompressibleInterIsoFoam
np="${OPENFOAM_NP:-3}"

[[ -x "$solver" ]] || { echo "Missing solver: $solver" >&2; exit 2; }
[[ "$run_dir" == /tmp/bh3-2d-qualification/medium_iso_valve ]] || exit 3
rm -rf -- "$run_dir"
mkdir -p "$run_dir"
cp -a "$case_root/case/." "$run_dir/"

python3 "$here/make_medium_config.py" \
    --source "$case_root/case_config.json" \
    --output "$run_dir/medium_case_config.json"
python3 "$case_root/make_blockmesh.py" \
    --config "$run_dir/medium_case_config.json" \
...
foamDictionary system/controlDict -entry maxCo -set 0.3 >/dev/null
foamDictionary system/controlDict -entry maxAlphaCo -set 0.25 >/dev/null
foamDictionary system/controlDict -entry maxDeltaT -set 0.0015 >/dev/null
