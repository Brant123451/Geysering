...
[[ -d "$RUN_DIR/processor0/10.5" ]] || {
    echo "Missing completed coarse-screen checkpoint at 10.5 s" >&2
    exit 3
}
[[ -d "$RUN_DIR/processor2" && ! -d "$RUN_DIR/processor3" ]] || {
    echo "Expected the preserved three-rank decomposition" >&2
    exit 4
}
