#!/usr/bin/env python3
"""Generate the dedicated B-H3 area-equivalent planar block mesh."""
...
def generate(config_path: Path, output_path: Path, stats_path: Path) -> None:
    ...
    m = cfg["mesh_m"]
    minimum = cfg.get("mesh_min_cells", {})
    ...
    nominal_riser_w = g["riser_inner_diameter"] ** 2 / pipe_d
    if abs(riser_w - nominal_riser_w) >= 1e-12:
        metadata = cfg.get("qualification_metadata", {})
        changed = metadata.get("single_changed_physical_model_parameter", {})
        calibrated_width_screen = (
            metadata.get("role")
            == "calibrated_single_parameter_effective_planar_riser_width_direction_screen"
            and metadata.get(
                "allow_non_area_equivalent_width_for_calibrated_sensitivity"
            )
            is True
            and changed.get("name") == "effective planar riser width"
            and abs(changed.get("nominal_area_equivalent_width_m", -1.0) - nominal_riser_w)
            < 1e-12
            and abs(changed.get("new_width_m", -1.0) - riser_w) < 1e-12
        )
        if not calibrated_width_screen:
            raise AssertionError(
                "non-area-equivalent riser width is allowed only for the "
                "explicit calibrated effective-width sensitivity"
            )
    ...
    nz_pipe = cells(pipe_d, m["pipe_dz"], int(minimum.get("pipe_z", 24)))
    nx_left = cells(x_rl, m["pipe_dx"], int(minimum.get("pipe_left_x", 80)))
    nx_junction = cells(riser_w, m["riser_dx"], int(minimum.get("riser_x", 8)))
    nx_mid = cells(x_valve - x_rr, m["pipe_dx"], int(minimum.get("pipe_mid_x", 80)))
    nx_pocket = cells(x_end - x_valve, m["pipe_dx"], int(minimum.get("pocket_x", 40)))
    nz_riser = cells(z_rim - z_crown, m["riser_dz"], int(minimum.get("riser_z", 200)))
    nx_ext_l = cells(x_rl - x_el, m["external_dx"], int(minimum.get("external_x", 12)))
    nx_ext_r = cells(x_er - x_rr, m["external_dx"], int(minimum.get("external_x", 12)))
    nz_ext = cells(z_top - z_rim, m["external_dz"], int(minimum.get("external_z", 80)))
...
{chr(10).join(faces)}
    ...
    text = f"""/* B-H3 paper-layout planar mesh; generated by make_blockmesh.py. */
FoamFile
{{
    version 2.0;
    format ascii;
