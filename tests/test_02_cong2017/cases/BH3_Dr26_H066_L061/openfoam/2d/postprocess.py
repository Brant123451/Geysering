#!/usr/bin/env python3
"""Extract B-H3 2D event metrics without fitting to the experiment."""
...
def read_riser_series(run: Path) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    for time in sorted(grouped):
        ...
        yint.append(min(air_to_water) if air_to_water else np.nan)
...
def log_status(run: Path) -> dict[str, object]:
    logs = [path for path in run.glob("log.solve*") if path.is_file()]
    if (run / "log.smoke").exists():
        logs.append(run / "log.smoke")
    logs.sort(key=lambda path: path.stat().st_mtime)
    segments = [path.read_text(errors="ignore") for path in logs]
    latest = segments[-1] if segments else ""
    times = [float(x) for x in re.findall(r"^Time = ([0-9.eE+-]+)", latest, re.M)]
    return {
        "last_log_time_s": times[-1] if times else None,
        "ended_normally": bool(re.search(r"^End\s*$", latest, re.M)),
        "fatal_error": "FOAM FATAL" in latest,
        "fatal_error_any_segment": any("FOAM FATAL" in text for text in segments),
        "latest_log": logs[-1].name if logs else None,
    }
...
def main() -> None:
    ...
    payload = {
        ...
        "run_id": "paper_bh3_tau0p2_areaeq",
        "dimension": "2D_planar_area_equivalent",
        "solver": "bh3CompressibleInterFoam-v2512",
    ...
    try:
        ...
        fig.suptitle("Cong 2017 B-H3 - OpenFOAM 2D paper-layout run")
        fig.tight_layout()
