#!/usr/bin/env python3
"""Extract B-H1 2D event metrics without fitting to the experiment."""
...
def log_status(run: Path) -> dict[str, object]:
    logs = [run / "log.solve", run / "log.solve.resume", run / "log.smoke"]
...
def main() -> None:
    ...
    run_id = args.run_id or cfg["simulation"]["primary_run_id"]
    ...
    payload = {
        "schema_version": 1,
        "case": "BH1_Dr16_H066_L061",
        "run_id": run_id,
        "dimension": "2D_planar_area_equivalent",
        "solver": "bh1CompressibleInterFoam-v2512",
        "paper_contract": cfg,
        "status": status,
        "n_riser_samples": int(t.size),
        "experiment": {
            "classification": experiment["classification"],
            "Ta_s": experiment["Ta_s"],
            "vfs_m_s": experiment["vfs_m_s"],
            "vint_m_s": experiment["vint_m_s"],
        },
        "model": model,
        "errors": {
            "Ta_s": None if ta is None else ta - experiment["Ta_s"],
            "vfs_m_s": None if vfs is None else vfs - experiment["vfs_m_s"],
            "vint_m_s": None if vint is None else vint - experiment["vint_m_s"],
    ...
    try:
        ...
        fig.suptitle("Cong 2017 B-H1 - OpenFOAM 2D paper-layout run")
        fig.tight_layout()
