...
def main():
    ...
    for n, k in enumerate(sel):
        ...
        Ar_frac = np.clip(np.asarray(alr) / (np.pi / 4 * case.Dr ** 2), 0.0, 1.0)
        ...
        # tower: water films by liquid fraction, the centre gap left grey = air;
        # the resolved gas core is additionally painted white so the bubble
        # remains visible inside the thin true-width column
        ...
        # ---------- tower zoom (gas-core width = local alpha_g, honest render) ----------
