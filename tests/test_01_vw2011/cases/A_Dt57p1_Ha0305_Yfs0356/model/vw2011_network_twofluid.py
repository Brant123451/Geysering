...
F_HAND = 0.99       # crown-front hand-off fraction of the region-mean layer depth.
                    ...
                    # against the region mean (asymptotic fill).  0.98 recovers
...
class NetworkCase:
    ...
    pocket_bleed: bool = False      # OPTIONAL connected-pocket pressure closure
                                    # (paper's film-flow picture): while gas climbs
                                    # the tower the pipe pocket is pneumatically
                                    # continuous with it, so the pocket pressure
                                    # cannot exceed the hydrostatic load of the water
                                    # STANDING ABOVE THE NOSE (wall film is shear-
                                    # supported, Batchelor film flow).  The pocket
                                    # bleeds excess mass into the tower cloud; the
                                    # visible level then swells during the climb by
                                    # the expansion volume (paper's rising Yfs).
                                    # A/B on Case A (2026-07-06): adds the swell but
                                    # the recorded head HUMPS to 0.68 near the catch
                                    # before decaying (baseline holds the plateau and
                                    # matches Fig.5 better overall) -> default OFF;
                                    # the paper's clean during-climb decay needs
                                    # their 0-D pocket-ODE topology (TPA), which a
                                    # stable 1D field closure could not reproduce.
    film_span_overlay: bool = False  # companion closure to pocket_bleed: inside the
                                    # coherent gas span the riser hydrostatic overlay
                                    # carries only (Yfs - itop) + the film share,
                                    # not the full column (see A/B note above).
    alpha_slug: float = 0.88        # coherent-pocket gas fraction used by the nose-
                                    # environment closure (thin wall film ~ alpha_TB)
...
def run_network(case: NetworkCase, verbose: bool = True) -> dict:
    ...
    def append_record(sample_t, alt_state, alr_state, mgt_state, mgr_res_state, rho_g_state):
        ...
        # NOTE on the visible level: the liquid repack packs the conserved liquid
        # AROUND the resolved gas, so _liquid_surface_height already returns the
        # swelled (visible) surface -- liquid top = (liquid volume + submerged
        # gas volume)/Ar.  An explicit "+ swell" here double-counted the gas
        # volume (fixed 2026-07-06).
    ...
    while t < case.t_end - 1e-12:
        ...
        # Riser hydrostatic overlay.  Plain 1D hydrostatics loads the FULL water
        # column onto every cell below the surface -- correct for a standing
        # column, wrong for the slug-flow climb: in the experiment the gas
        # ascending the tower is one coherent Taylor pocket whose wall film is
        # SHEAR-SUPPORTED (Batchelor film flow), so the water riding ABOVE the
        # nose is the only weight the gas span carries.  With the full-column
        # overlay the recorded head holds its plateau to breakthrough (cliff);
        # the experiment decays DURING the climb: H* ~ (Yfs - Yint), the
        # paper's Fig.5 T*=7.5..9.3 / Fig.10 ramp.  When a coherent climb is
        # underway (pocket_bleed closure, >= 3 gas cells), cells inside the gas
        # span [0, itop] carry (Yfs - itop) + the film share of the span above
        # them; cells above itop carry the normal (Yfs - z).
        head_r = np.maximum(Yfs - zr, 0.0)
        if case.film_span_overlay:
            m_thr_bl = 0.05 * rho_atm * Ar * dz
            gas_idx_bl = np.where(Mgrs > m_thr_bl)[0]
            if gas_idx_bl.size >= 3:
                # No vent_latched cutoff: after breakthrough the whole column IS
                # the film-supported topology, and the hard switch back to the
                # full-column overlay slammed the water onto the pocket (recorded
                # +1.0 L hammer spike right at the latch).
                itop_bl = min(float(zr[gas_idx_bl[-1]] + 0.5 * dz), float(Yfs))
                in_span = zr < itop_bl
                head_span = (np.maximum(Yfs - itop_bl, 0.0)
                             + (1.0 - case.alpha_slug) * np.maximum(itop_bl - zr, 0.0))
                head_r = np.where(in_span, head_span, head_r)
        ...
        if alpha_g_j > case.tower_entry_alpha_min:
            ...
            # (A full coherent-slug topology closure -- consolidation, slug-bore
            # exchange, material-interface faces -- was also tried 2026-07-06 and
            # reverted: it destabilised the tuned junction liquid dynamics.  The
            # paper's during-climb pressure decay is instead captured by the
            # pocket_bleed closure below.)
            m_up = min(rho_g_j * alpha_g_j * Ar * u_in * dt, 0.5 * max(Mgt_new[jx], 0.0))
            old_mgt = max(Mgt_new[jx], 1.0e-14)
            Mgt_new[jx] -= m_up
            Jgt_new[jx] *= max(Mgt_new[jx], 0.0) / old_mgt
            Mgr_new[0] += m_up
            Mgrs_new[0] += m_up
            Jgrs_new[0] += m_up * u_in
            ...
            Alr_new[0] -= V_ex / dz
            Alt_new[jx] += V_ex / dx
        # ---- pneumatic bleed (pocket_bleed): connected-pocket pressure closure.
        # In the experiment the gas climbing the tower IS the pipe pocket poking
        # up through the T (Taylor pocket, wall film draining around it) -- one
        # connected gas body.  Its pressure therefore cannot exceed the
        # hydrostatic load of the water standing ABOVE THE NOSE (the film beside
        # the gas is shear-supported, Batchelor film flow): as the nose climbs,
        # the pocket head decays toward H* ~ (Yfs - Yint) -- the paper's gradual
        # Fig.5/Fig.10 decay.  The 1D drift-flux cloud cannot represent that
        # topology (its interspersed liquid loads the base with the FULL column
        # weight, holding the plateau to breakthrough), so the pressure
        # communication is imposed as a closure: relax the pipe pocket toward
        # the coherent-pocket nose environment by bleeding its excess mass into
        # the tower cloud (isothermal; volume held by the surrounding water on
        # the bleed timescale).  The environment uses the CLOUD's validated nose
        # position itop and visible level, but the slug-topology film weight
        # (1 - alpha_slug).
        if (case.pocket_bleed and not vent_latched and m_poc > 0.0 and i0_poc >= 0):
            rho_pre_bl = np.maximum(Pr / (R_GAS * T_GAS), rho_atm)
            a_pre_bl = np.clip(Mgrs / np.maximum(rho_pre_bl * Ar * dz, 1.0e-12), 0.0, 0.98)
            gas_idx_bl = np.where(a_pre_bl > 0.05)[0]
            if gas_idx_bl.size >= 3:
                liq_bl = float(np.sum(np.clip(Alr, 0.0, Ar))) * dz
                vgas_bl = float(np.sum(a_pre_bl * Ar * dz))
                Yvis_bl = min((liq_bl + vgas_bl) / Ar, float(zr[-1] + 0.5 * dz))
                itop_bl = min(float(zr[gas_idx_bl[-1]] + 0.5 * dz), Yvis_bl)
                P_env_bl = (P_ATM + RHO_L * G * (max(Yvis_bl - itop_bl, 0.0)
                                                 + (1.0 - case.alpha_slug) * itop_bl))
                # Hand off to the breakthrough vent near the catch: as the nose
                # closes on the surface the environment head plunges toward the
                # film share alone, and a bleed racing it yanked the pocket down
                # so hard the tower crashed 0.56 -> 0.20 within 0.3 s (tau 0.1)
                # or hammer-spiked +0.85 (tau 0.2).  The bleed's job is the
                # DURING-CLIMB decay; the collapse at the catch belongs to the
                # vent, which is rate-limited and latched.
                if Pj > P_env_bl and (Yvis_bl - itop_bl) > 0.10 * float(zr[-1]):
                    # tau tuning record (Case A, WL=0.356 / WL=0.254):
                    #   0.1 -- pocket tracks the environment (recorded head DECAYS
                    #         through the climb, the paper shape) but the catch
                    #         arrives 0.7 T* early and the tower drains through
                    #         the Fig.7 window (0.607 -> 0.13 by T*=10.2);
                    #   0.4 -- head decay only partial (mild rise near the catch)
                    #         but every validated baseline metric is preserved
                    #         (liftoff 6.5, catch 8.44, plateau 0.527, no geyser)
                    #         AND the level physically swells during the climb
                    #         (0.59 -> 0.635; experiment 0.55 -> 0.65).
                    # 0.4 is the production setting: it adds the paper's swell
                    # without giving up the validated timing; the full during-
                    # climb decay is out of reach of a stable 1D closure here
                    # (the paper's own model handles it as a 0-D pocket ODE).
                    tau_bleed = 0.4
                    dP_bl = (Pj - P_env_bl) * min(dt / tau_bleed, 0.5)
                    m_reg_now = float(np.sum(Mgt_new[i0_poc:i1_poc]))
                    dm_bl = min(m_poc * dP_bl / max(P_poc, P_ATM), 0.2 * m_reg_now)
                    if dm_bl > 0.0 and m_reg_now > 1.0e-12:
                        f_bl = 1.0 - dm_bl / m_reg_now
                        Mgt_new[i0_poc:i1_poc] *= f_bl
                        Jgt_new[i0_poc:i1_poc] *= f_bl
                        Mgr_new[0] += dm_bl
                        Mgrs_new[0] += dm_bl
                        Jgrs_new[0] += dm_bl * 0.35 * math.sqrt(
