...
local central-upwind finite-volume flux with a donor-cell draining limiter.
Zero area is a valid state: no artificial water film is inserted.  The
extension is deliberately isolated in :func:`central_upwind_wet_dry_step` so
that it cannot be mistaken for the paper's shock-fitting equations.
...
def central_upwind_wet_dry_step(
    ...
    *,
    ...
    """Advance one positivity-preserving wet/dry step.
    ...
    # Donor-cell draining limiter.  A face may not remove more water than its
    # donor owns during this time step.  Scaling the paired momentum flux
    # preserves a bounded donor velocity while guaranteeing A >= 0.
    outgoing = np.maximum(mass_flux[1:], 0.0) + np.maximum(-mass_flux[:-1], 0.0)
    theta = np.ones(ncell)
    draining = outgoing > 0.0
    theta[draining] = np.minimum(
        1.0,
        area[draining] * dx / (dt * outgoing[draining]),
    )
    face_scale = np.ones(ncell + 1)
    internal_flux = mass_flux[1:-1]
    internal_faces = np.arange(1, ncell)
    donor = np.where(internal_flux >= 0.0, internal_faces - 1, internal_faces)
    face_scale[1:-1] = theta[donor]
    mass_flux *= face_scale
    momentum_flux *= face_scale
...
class Tosan2021HorizontalShockFit:
    ...
    def _connected_gas_volume(
        ...
        liquid_local = liquid.copy()
        liquid_local[partial] = float(
            np.clip(boundary_area, 0.0, self.section.full_area)
        )
        return float(np.sum(overlap * (self.section.full_area - liquid_local)))
    ...
    def _step_once(
        ...
        free_fraction = float(
            np.clip((interface_new - cut_left) / self.dx, 0.0, 1.0)
        )
        ...
        area_new[cut_index] = (
            ...
            + (1.0 - free_fraction) * pressurised_cut_area
        )
        discharge_new[cut_index] = (
            free_fraction * boundary_discharge
            + (1.0 - free_fraction) * pressurised_cut_discharge
        )
        ...
        return HorizontalState(
            ...
            wetting_front_x=self._wetting_front(
                wetdry.area, state.wetting_front_x
            ),
            vented=vented,
