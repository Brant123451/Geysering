...
# Through the physical tower and the external-plume centreline above the
# open rim (y=0.657 m). Above-rim centreline heights are qualitative because
# this is an area-equivalent planar surrogate for a circular tower jet.
PROBE_Y = np.arange(0.052, 0.943, 0.010)
...
def main() -> None:
    ...
    pressure_exp = np.genfromtxt(
        DATA / "fig6_caseB_Hstar_band.csv",
        delimiter=",",
        names=True,
    )
    levels_exp = np.genfromtxt(
        DATA / "fig8_caseB_levels.csv",
        delimiter=",",
        names=True,
        dtype=None,
        encoding="utf-8",
    ...
    metrics = {
        ...
        "comparison_targets": {
            ...
            "figure_levels": "Fig.8",
            "pressure_plateau_Hstar_exp_median_approx": 0.70,
        },
        "paper_inputs": {
            "D_m": 0.094,
            "Dt_m": 0.0127,
        ...
        "caveat": (
            ...
            "(paper: manual <1 s). The physical tower ends at y=0.657 m and "
            "opens into a 0.24 m wide by 0.60 m high atmospheric box. Above-rim "
            "centreline levels diagnose a qualitative planar free jet, not a "
            "quantitative circular-plume diameter."
        ),
