...
HERE = Path(os.environ.get("CASEB_OPENFOAM_CASE", SCRIPT_DIR))
CASE_ROOT = Path(os.environ.get("CASEB_CASE_ROOT", HERE.parent.parent))
DATA = CASE_ROOT / "data" / "digitized"
OUT = Path(os.environ.get("CASEB_OUTPUT_DIR", HERE / "outputs"))
...
def main() -> None:
    ...
    metrics = {
        ...
        "dimension": "2d_planar_open_rim_matched_physics",
        ...
        "caveat": (
            "Planar 2-D keeps paper lengths, D, Ha0, Yfs0 and the physical open "
            "rim. The area-equivalent tower width W=Dt^2/D preserves branch "
            "capacity but not circular wall shear; the archived 2-D no-slip wall "
            "treatment is retained without a second bulk-Darcy source. Gas/liquid "
            "temperature is constrained to 293.15 K and the 0.25-s valve opening "
            "with Kopen=2 is represented by piecewise porous resistance. Sigma=0 "
            "is retained to avoid capillary lock in the artificial planar slit."
        ),
        "planar_tower_width_m": 0.0127**2 / 0.094,
        "sigma": 0.0,
        "matched_physics": {
            "isothermal_temperature_K": 293.15,
            "valve_open_time_s": 0.25,
            "valve_fully_open_K": 2.0,
            "bulk_Darcy_source_added": False,
            "bulk_Darcy_reason": "avoid double counting resolved 2-D no-slip wall shear",
        },
    }
