#!/bin/bash
# Fresh 0.12 s rim-onset screen: stack nCorrectors=3 + nOuterCorrectors=2.
set -euo pipefail
cd "$(dirname "$0")"
...
export CASEB_N_OUTER_CORRECTORS=2
...
echo "START_STACK_NC3_NO2 $(date -u +%Y-%m-%dT%H:%M:%SZ)"
./Allclean
./Allrun
echo "END_STACK_NC3_NO2 exit:$? $(date -u +%Y-%m-%dT%H:%M:%SZ)"
