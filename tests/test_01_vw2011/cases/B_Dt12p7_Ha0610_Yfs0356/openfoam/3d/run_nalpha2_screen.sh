#!/bin/bash
# Fresh 0.12 s rim-onset screen: isolate CASEB_N_ALPHA_CORR=2 on admitted baseline.
# Leaves exhausted PIMPLE / non-orthogonal / maxCo=0.10 axes untouched.
set -euo pipefail
cd "$(dirname "$0")"
...
export OPENFOAM_NP="${OPENFOAM_NP:-12}"

echo "START_NALPHA2 $(date -u +%Y-%m-%dT%H:%M:%SZ)"
# /mnt/e is a Windows mount; strip CR so bash can execute scripts reliably.
...
bash Allclean
bash Allrun
echo "END_NALPHA2 exit:$? $(date -u +%Y-%m-%dT%H:%M:%SZ)"
