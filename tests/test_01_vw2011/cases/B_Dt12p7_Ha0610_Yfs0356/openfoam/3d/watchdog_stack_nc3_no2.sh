#!/bin/bash
# 20-minute health watchdog for stacked nCorrectors=3 + nOuterCorrectors=2.
...
LOG="$CASE_DIR/log.watchdog_stack_nc3_no2"
...
restart_screen() {
  ...
  pkill -f '[.]/run_stack_nc3_no2_screen.sh' 2>/dev/null || true
  sleep 5
  ...
  if [[ -d processor0 && -f outputs/runtime/run_manifest.json ]]; then
    ...
    # Stack screen is a fresh experiment; if incomplete, resume same controls.
    echo "WATCHDOG_ACTION resume_via_Allrun.resume" | tee -a "$LOG"
    # Ensure end time remains 0.12 with stacked controls from manifest.
    nohup bash -lc '
      set -euo pipefail
      ...
      set -u
      ./Allrun.resume
    ' >> log.stack_nc3_no2_screen 2>&1 &
  else
    echo "WATCHDOG_ACTION full_rerun_via_run_stack" | tee -a "$LOG"
    nohup ./run_stack_nc3_no2_screen.sh >> log.stack_nc3_no2_screen 2>&1 &
  fi
  sleep 20
}
