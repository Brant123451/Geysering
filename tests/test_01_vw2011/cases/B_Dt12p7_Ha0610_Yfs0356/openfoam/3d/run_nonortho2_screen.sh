#!/bin/bash
# Fresh 0.12 s rim-onset screen: isolate nNonOrthogonalCorrectors=2 on admitted baseline.
set -euo pipefail
cd "$(dirname "$0")"
...
export CASEB_N_NON_ORTHOGONAL_CORRECTORS=2
...
echo "START_NONORTHO2 $(date -u +%Y-%m-%dT%H:%M:%SZ)"
./Allclean
./Allrun
echo "END_NONORTHO2 exit:$? $(date -u +%Y-%m-%dT%H:%M:%SZ)"
