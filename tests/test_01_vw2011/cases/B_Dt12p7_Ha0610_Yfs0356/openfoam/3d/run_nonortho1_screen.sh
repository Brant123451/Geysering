#!/bin/bash
# Fresh 0.12 s rim-onset screen: isolate nNonOrthogonalCorrectors=1 on admitted baseline.
set -euo pipefail
cd "$(dirname "$0")"
...
export CASEB_N_NON_ORTHOGONAL_CORRECTORS=1
...
echo "START_NONORTHO1 $(date -u +%Y-%m-%dT%H:%M:%SZ)"
./Allclean
./Allrun
echo "END_NONORTHO1 exit:$? $(date -u +%Y-%m-%dT%H:%M:%SZ)"
