...
31. `nCorrectors=3` 的全新 0.12 s 筛查已完成并被拒绝：
    ...
    0.12 s rim-onset 筛查。
32. `nOuterCorrectors=2` 的全新 0.12 s 筛查已完成并被拒绝：
    - solver 达到 0.119996 s；global/interface Co 为 0.160/0.153；
    - 0.050 s 曾短暂出现 rim 纯气热点（U=0.795），随后回到自由面至 0.100 s；
    - 0.110 s 同一 rim 单元再次锁定（U=1.691），0.120 s 增至 1.841，曲率为 0；
    - \(H^*\) peak-to-peak 仅 0.000844，压力 alone 会再次误判。
    不得单独把 `nOuterCorrectors=2` 写入 baseline。下一步把此前两个“仅推迟”
...
34. `nNonOrthogonalCorrectors=1` 的全新 0.12 s 筛查已完成并被拒绝：
    - solver 达到 0.120003 s（中途约 0.1017 s 停滞，已从 0.099998 dump 续算）；
      global/interface Co 为 0.162/0.152；
    - 直至 0.100 s 热点仍在自由面；0.110 s 跳到 rim 纯气（U=1.690），
      0.120 s 增至 1.841，曲率为 0；
    - \(H^*\) peak-to-peak 仅 0.001016，压力 alone 会再次误判；
...
36. `maxCo=0.10` 的全新 0.12 s 筛查已完成并被拒绝：
    - solver 达到 0.120002 s；峰值/运行 Co 约 0.109/0.100；
    - 直至 0.060 s 热点仍在自由面；从 0.070 s 起同一 rim 纯气单元锁定并单调
      增长 \(1.099\rightarrow1.242\rightarrow1.395\rightarrow1.548\rightarrow1.698\rightarrow1.849\)
      m/s，曲率为 0；
    - \(H^*\) peak-to-peak 仅 0.001435，压力 alone 会再次误判；
    - 相对 `maxCo=0.15` 基线拒绝（rim ~0.117 s）反而更早 onset，终态速度几乎相同。
    不得把 `maxCo=0.10` 写入 baseline，也勿再单独继续压小 `maxCo`。
...
- CASEB_MAX_CO=0.15（`=0.10` 的 0.12 s rim 筛查已拒绝；勿再单独压小；`0.30` 仍为必做敏感性）
