#!/bin/bash
# Fresh 0.12 s rim-onset screen: isolate CASEB_MAX_CO=0.10 on admitted baseline.
set -euo pipefail
cd "$(dirname "$0")"
...
echo "START_MAXCO010 $(date -u +%Y-%m-%dT%H:%M:%SZ)"
./Allclean
./Allrun
echo "END_MAXCO010 exit:$? $(date -u +%Y-%m-%dT%H:%M:%SZ)"
