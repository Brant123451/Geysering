#!/bin/bash
# 20-minute health watchdog for maxCo=0.10 rim-onset screen.
set -euo pipefail
CASE_DIR="/workspace/tests/test_01_vw2011/cases/B_Dt12p7_Ha0610_Yfs0356/openfoam/3d"
LOG="$CASE_DIR/log.watchdog_maxco010"
...
restart_screen() {
  ...
  pkill -f '[.]/run_maxco010_screen.sh' 2>/dev/null || true
  sleep 5
  if [[ -d processor0 && -f outputs/runtime/run_manifest.json ]]; then
    echo "WATCHDOG_ACTION resume_via_Allrun.resume" | tee -a "$LOG"
    tmux -f /exec-daemon/tmux.portal.conf has-session -t '=caseb-maxco010-resume' 2>/dev/null || \
      tmux -f /exec-daemon/tmux.portal.conf new-session -d -s caseb-maxco010-resume -c "$CASE_DIR" -- bash -l
    tmux -f /exec-daemon/tmux.portal.conf send-keys -t 'caseb-maxco010-resume:0.0' './Allrun.resume' C-m
  else
    echo "WATCHDOG_ACTION full_rerun" | tee -a "$LOG"
    tmux -f /exec-daemon/tmux.portal.conf has-session -t '=caseb-maxco010-0p12' 2>/dev/null || \
      tmux -f /exec-daemon/tmux.portal.conf new-session -d -s caseb-maxco010-0p12 -c "$CASE_DIR" -- bash -l
    tmux -f /exec-daemon/tmux.portal.conf send-keys -t 'caseb-maxco010-0p12:0.0' './run_maxco010_screen.sh' C-m
  fi
  sleep 20
}
