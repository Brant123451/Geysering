...
def write(path: Path, text: str, executable: bool = False) -> None:
    ...
    # OpenFOAM runs in WSL; force LF so generated shell scripts keep a valid
    # /bin/bash shebang even when this generator is executed from Windows.
...
def generate_block_mesh_v2(paper: dict, fine: bool = True) -> tuple[str, dict]:
    ...
    The upstream invert falls toward the chamber at the paper's 1:100 slope.
    Two skewed hexahedral bands meet the chamber conformally at z=drop and
    z=drop+D. Pipe diameters, lengths, chamber, riser, and gate opening ratio
    otherwise match the declared C9 parameters.
    """
    import re
    ...
    # z stations (conformal). The upstream pipe falls toward the chamber.
    # At x=-Lu its invert/crown are Lu*slope above their values at x=0.
    z_u0, z_u1 = drop, drop + du          # chamber-side: 0.18 .. 0.38
    z_u0_in = z_u0 + lu * slope           # inlet-side: 0.238
    z_u1_in = z_u1 + lu * slope           # inlet-side: 0.438
    ...
    def hex_quad(x0_, z0_lo, z0_hi, x1_, z1_lo, z1_hi, nx, nz) -> str:
        """Extrude an x-z quadrilateral by THICKNESS in y."""
        a = V(x0_, z0_lo)
        b = V(x1_, z1_lo)
        c = V(x1_, z1_hi)
        d = V(x0_, z0_hi)
    ...
    def hex_xz(x0_, z0_, x1_, z1_, nx, nz) -> str:
        return hex_quad(x0_, z0_, z1_, x1_, z0_, z1_, nx, nz)
    ...
    # Upstream. The internal split follows the same slope and reaches z=0.28
    # at the chamber, so both bands remain conformal with the chamber blocks.
    split_fraction = (z_d1 - z_u0) / du
    z_split_in = z_u0_in + split_fraction * du
    blocks.append(hex_quad(x_in, z_u0_in, z_split_in, x0, z_u0, z_d1, nx_u, nz_u_lo))
    blocks.append(hex_quad(x_in, z_split_in, z_u1_in, x0, z_d1, z_u1, nx_u, nz_u_hi))
    ...
    inlet = [
        edge_face(x_in, z_u0_in, x_in, z_split_in),
        edge_face(x_in, z_split_in, x_in, z_u1_in),
    ]
    ...
    walls = [
        edge_face(x_in, z_u0_in, x0, z_u0),
        edge_face(x_in, z_u1_in, x0, z_u1, flip=True),
...
"""
    meta = {
        ...
        "upstream_inlet_invert_z_m": z_u0_in,
        "upstream_chamber_invert_z_m": z_u0,
        "upstream_fall_toward_chamber_m": lu * slope,
        "note_slope": "paper 1:100 upstream fall is represented by skewed conformal blocks",
    }
    return text, meta
...
""",
    ...
    # setExprFields is used after setFields to impose the continuous
    # hydrostatic p(z) in water. This second dictionary then restores the gas
    # pressures, which must not use the water-density hydrostatic expression.
    write(
        CASE / "system/setFieldsGasDict",
        foam_header("dictionary", "setFieldsGasDict")
        + f"""defaultFieldValues
(
);
...
def make_control(end: float, write_interval: float = 0.25) -> str:
    ...
    pt4_crown = (
        ...
        - x_pt4 * paper["upstream_slope"]
    )
    pt4_z = pt4_crown - 0.002
    return (
...
{{
    ...
    {{
        ...
        (
            (0.15 0 1.25)   // PT1
            (0.15 0 0.448)  // PT2, 2 mm inside chamber top tap
            (0.15 0 0.02)   // PT3
            ({x_pt4:.2f} 0 {pt4_z:.6g}) // PT4, 2 mm inside the sloping crown
        );
...
def generate_run_scripts(np: int = 4, hydrostatic_p_expression: str = "") -> None:
    write(
...
# C9 starts from an established Q0=25 L/s flow, not stagnant water. Close the
# atmosphere only while potentialFoam builds a divergence-free pipe-to-gate
# velocity field; then restore the transient open-atmosphere condition.
foamDictionary 0/U -entry boundaryField.atmosphere.type -set fixedValue
potentialFoam -pName p_rgh -initialiseUBCs -writephi > log.potentialFoam 2>&1
foamDictionary 0/U -entry boundaryField.atmosphere.type -set pressureInletOutletVelocity
setFields > log.setFields 2>&1
setExprFields -time 0 -field p \\
  -expression '""" + hydrostatic_p_expression + """' -keepPatches > log.setExprFields 2>&1
setFields -dict system/setFieldsGasDict > log.setFieldsGas 2>&1
...
def main() -> None:
    ...
    hydrostatic_p_expression = (
        f"{paper['atmospheric_pressure_Pa']}+"
        f"{model['water_density_kg_m3']}*9.81*"
        f"({paper['chamber_height_m'] + paper['initial_riser_column_m']}-pos().z())"
    )
    generate_run_scripts(args.np, hydrostatic_p_expression)
