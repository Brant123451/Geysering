...
{{
    ...
    {{
        ...
        // advect sigma with phiVol = phi_air / max((alpha*rho)_f,
        // resolveAlpha*rho_f), limited by MULES against
        // [0, max(alpha*rho, sigma)].  Face floor uses resolveAlpha (1e-3),
        // not residualAlpha: thinner floors explode phiVol / go negative.
        // Recover s = sigma/(alpha*rho) for output; mild s>1 allowed.
...
{os.linesep.join(deep_crown_points)}
    ...
    {{
        ...
        libs            (fieldFunctionObjects);
        operation       volIntegrate;
        fields          (pocketBodyTracerMassSource);
        writeControl    adjustableRunTime;
        writeInterval   0.01;
        writeFields     false;
    }}
