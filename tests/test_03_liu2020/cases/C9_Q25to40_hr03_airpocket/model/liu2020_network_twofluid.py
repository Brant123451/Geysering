...
CASE-C9 COPY (this folder's frozen copy; Series-C extensions over the
Case-B3 copy, all initial/boundary-condition switches, no retuning):

3. series_c mode: the upstream pipe starts as the Q0 free-surface profile
   whose crown air is ONE SEALED pocket (the paper: "the air initially
   occupying the upper part of the upstream pipe was entrapped as an air
   pocket"), with a short flooded plug at the inlet (the supply enters
   from a pressurized tank through the ball valve, so the pipe entrance
   is drowned and the pocket cannot vent backward).  Gas regions touching
   the chamber are NOT auto-vented in this mode: the pocket connects to
   the chamber only through the thin crown air layer whose bubble-leak
   rate is far below the instant-vent idealization.  ROUND-1 SCOPE: the
   pocket stays sealed for the whole run -- this reproduces PHASE 1 of
   the measured record (the hydraulic-transient geysers, the paper's own
   analytic model Eq. (7) addresses exactly this phase under the same
   no-release assumption); the phase-2 pocket-body arrival and the
   bubble/slug geyser train 3--8 are DECLARED beyond this reduction.

4. the riser starts with the documented initial column hr0 (held during
   warm-up, released at t = 0) and the Series-C tail gate is a throttled
   orifice: the outlet momentum face uses the effective loss
   (Ad/A_gate)^2 - 1 of the gate contraction.  A_gate is rated ONCE from
   the DOCUMENTED INITIAL CONDITION (PT2 = 2.97 kPa at Q0 = 25 L/s with
   hr0 = 0.30 m => chamber HGL 0.75 m) against the case-B tail level
   H_tail = Dd; the final steady state and all transients are predictions.
"""
...
class LiuCase:
    ...
    H_tail: float = 0.28         # Series-B downstream boundary: constant
                                 ...
                                 # final-state outcome.
    series_c: bool = False       # Series-C mode: sealed upstream crown pocket
                                 # + initial riser column + throttled gate
    hr0: float = 0.0             # documented initial riser column [m]
    x_nose: float = 1.0          # pocket nose stand-off from the chamber
                                 # wall [m]: cells within x_nose of the wall
                                 # start full (the thin connecting crown film
                                 # of Fig. 2(c) is below resolution); the
                                 # pocket body occupies the crown upstream of
                                 # it.  Geometric assumption, declared.
    no_pocket: bool = False      # Series-C ALL-WATER variant: the upstream
                                 # pipe starts full (no trapped pocket).
                                 # This is exactly the assumption set of the
                                 # paper's own analytic model Eq. (7)
                                 # ("pipes and chamber initially filled with
                                 # water"), used for the three-way
                                 # model/experiment/analytic comparison of
                                 # phase 1.  The sealed-pocket variant is
                                 # valid through phase 1 (the cushioning is
                                 # visible) but drifts once sustained
                                 # compression squeezes the pocket through
                                 # the reduced solver's full/gassy threshold
                                 # -- resolving that front needs the
                                 # companion framework's gas momentum + RTRP
                                 # closure (declared limit, see README).
    A_gate: float = 0.0          # effective tail-gate orifice area [m2]; 0 =
                                 # rate it from the documented initial state:
                                 # chamber HGL(Q0) = Hc + hr0 (PT2 initial),
                                 # A_gate = Q0 / sqrt(2 g (HGL - dz_fric -
                                 # u_d^2/2g - H_tail)).  For C9 this gives
                                 # ~0.0084 m2 (13.7% of the pipe section) --
                                 # derived from the INITIAL condition only.
    L_plug: float = 0.0          # flooded inlet plug length [m].  Default 0:
                                 # the pocket extends TO the inlet face and is
                                 # sealed there by policy (the supply enters
                                 # from a pressurized tank through the ball
                                 # valve, so air cannot escape upstream); the
                                 # inlet face flux is prescribed Q(t) anyway.
                                 # A discrete full-water plug was tried and
                                 # REVERTED: the sharp full/stratified area
                                 # jump at the plug face put the elastic wave
                                 # speed into the Rusanov mass diffusion
                                 # (~0.2 m3/s of spurious flux) and crushed
                                 # the pocket during warm-up.
...
def _run_case_composite(case: LiuCase, verbose: bool = True) -> dict:
    ...
    n_plug = int(round(case.L_plug / dx))
    if case.series_c:
        # ---- Series-C initial field (documented ICs + one geometric
        # assumption, both declared) ----
        # chamber + downstream full; riser holds the documented column hr0;
        # the chamber HGL is therefore HGL0 = Hc + hr0 above the chamber
        # bottom (PT2/PT3 initial readings confirm this directly).
        #
        # Upstream crown pocket: ONE sealed region in piezometric
        # equilibrium with HGL0, its nose ending x_nose before the chamber
        # wall (Fig. 2(c): pocket body upstream, only a thin crown film --
        # below this reduction's resolution -- connects it to the chamber).
        # Under the pocket the water depth follows
        #   h(x) = HGL0 - zinv(x) - Pp0/(rho g),
        # with Pp0 chosen so the pocket wedges out exactly at the nose.
        # Cells past the nose run full WITH the initial elastic overfill
        # that puts their piezometric head at HGL0 too -- without it the
        # near-full cells sat at hydrostatic pressure only, the 0.37 m
        # piezometric hole sucked the pocket water toward the chamber, and
        # the warm-up crushed the pocket to 138 kPa (first-run failure).
        # The paper does not report the trapped volume (part of the Q0
        # free-surface air escaped through the riser while the tailgate
        # was closed), so the pocket size follows from x_nose; the
        # documented pressures (PT4 initial ~4 kPa) are insensitive to it.
        HGL0 = case.Hc + case.hr0
        h_strat = float(np.interp(0.95 * case.Au, a_u, h_u))
        z_nose = case.drop + case.slope_u * case.x_nose
        Pp0 = RHO_L * G * max(HGL0 - z_nose - h_strat, 0.05)
        for i in range(N):
            if i < ic0:
                hh = HGL0 - zinv[i] - Pp0 / (RHO_L * G)
                if hh < h_strat and i >= n_plug and not case.no_pocket:
                    Al[i] = float(np.interp(max(hh, 0.02), h_u, a_u))
                    continue
                dfull = case.Du
            elif i < ic1:
                dfull = case.Hc
                ...
                dfull = case.Dd
            # full cell: elastic overfill so that piez = HGL0 everywhere
            ovh = max(HGL0 - zinv[i] - dfull, 0.0)
            Al[i] = A_full[i] * (1.0 + G * ovh / case.a_wh ** 2)
        void0 = np.maximum(A_full - Al, 0.0) * dx
        Mg = np.zeros(N)
        Mg[:ic0] = ((P_ATM + Pp0) / (R_GAS * T_GAS)) * void0[:ic0]
    gas_vented = 0.0
    hr = case.hr0 if case.series_c else 0.0
    wr = 0.0                 # riser column velocity [m/s] (rigid-column ODE)
    wr_eject = 0.0           # max column-top velocity while erupting
    geyser_hit = False
    a2 = case.a_wh ** 2
    A_gate = case.A_gate
    if case.series_c and A_gate <= 0.0:
        # rate the throttled tail gate ONCE from the documented initial
        # state (friction and velocity head at Q0 are ~2% and neglected)
        A_gate = case.Q0 / math.sqrt(2.0 * G * (case.Hc + case.hr0 - case.H_tail))

    rec = dict(t=[], PT1=[], PT2=[], PT3=[], PT4=[], hr=[], S=[], Qin=[], Qout=[],
               Qjin=[], Qjout=[],
    ...
    i_pt4 = max(ic0 - int(round(0.30 / dx)), 0)   # upstream crown, 0.30 m
                                                  # before the chamber wall
    out_dt = 0.01
    frame_dt = 0.10
    next_out = 0.0
    next_frame = 0.0
    ...
    while t < t_total - 1e-12:
        ...
        P = RHO_L * G * h
        gassy = frac < 0.97
        sealed_cells = np.zeros(N, dtype=bool)
        for (i0, i1) in _regions(gassy):
            touches_chamber = not (i1 <= ic0 or i0 >= ic1)
            if case.series_c and i1 <= ic0 and i0 >= n_plug:
                sealed_reg = True     # upstream crown pocket: STAYS sealed
            else:
                sealed_reg = not (touches_chamber or i0 == 0 or i1 == N)
            if not sealed_reg:
                continue                                  # vented region
            sealed_cells[i0:i1] = True
            m_reg = float(np.sum(Mg[i0:i1]))
            v_reg = float(np.sum(np.maximum(A_full[i0:i1] - Al[i0:i1], 1e-6)) * dx)
            P_reg = m_reg * R_GAS * T_GAS / max(v_reg, 1e-12) - P_ATM
            # cap: a near-annihilated fragment (all cells at frac 0.96-0.97,
            # never marked full, void -> 0) otherwise explodes the EOS; one
            # atmosphere of gauge overpressure is far beyond any physical
            # driving head in this rig
            P[i0:i1] += float(np.clip(P_reg, -0.5 * P_ATM, 1.0 * P_ATM))
        ...
        if case.series_c:
            # Series-C outlet: THROTTLED tail gate = orifice of effective
            # area A_gate against the tank level, quasi-steady with a 50 ms
            # relaxation (the gate loss dominates the face dynamics, so the
            # momentum-face closure of the Series-B outfall degenerates to
            # this rating).
            hd_gate = max(piez[-1] - case.H_tail, 0.0)
            Q_g = A_gate * math.sqrt(2.0 * G * hd_gate)
            Qout_f += (Q_g - Qout_f) * min(dt / 0.05, 1.0)
            Qout = min(Qout_f, 0.9 * Al[-1] * dx / dt)
        elif case.downstream_full:
        ...
        sfc = np.maximum(np.abs(u[:-1]) + c[:-1], np.abs(u[1:]) + c[1:])
        if case.series_c:
            # CASE-C9 copy: faces adjoining a gassy cell (the pocket nose /
            # tail) use the GRAVITY-WAVE celerity, not the elastic one.
            # Volume does not cross a pocket front at the acoustic speed;
            # with sfc = a_wh = 40 the Rusanov mass dissipation across the
            # standing nose (area jump ~0.05 A) pumped ~30 L/s of spurious
            # flux into the pocket zone and crushed the pocket during
            # warm-up.  This face rule is the reduced-solver stand-in for
            # the companion framework's Regime-Transition Riemann closure,
            # which replaces exactly this face.
            c_grav = np.sqrt(G * np.maximum(h, 1e-3))
            sfc_g = np.maximum(np.abs(u[:-1]) + c_grav[:-1],
                               np.abs(u[1:]) + c_grav[1:])
            near_gas = gassy[:-1] | gassy[1:]
            sfc = np.where(near_gas, sfc_g, sfc)
        ...
        if case.series_c:
            # a SEALED pocket boundary is a pressure-coupled interface, not
            # a dry filling front: keep the centered gradient there.  The
            # front limiter mis-fired on the pocket nose (opposite one-sided
            # piez slopes -> minmod = 0 -> no pressure force), so the pocket
            # over-pressure could not push the downstream water column away
            # and the inflow compressed the pocket without bound (82 kPa by
            # t = 6 s against the ~11 kPa hydrostatic equilibrium).
            near_sealed = (sealed_cells[2:] | sealed_cells[1:-1]
                           | sealed_cells[:-2])
            front = front & ~near_sealed
        ...
        if t < t_ramp0:
            # warm-up: hold the riser at its DOCUMENTED initial state (empty
            # for Series B, the preset column hr0 for Series C) -- settling
            # transients must not launch a spurious column.
            hr = case.hr0 if case.series_c else 0.0
            wr = 0.0
        ...
        if t_rep >= next_out - 1e-12:
            ...
            # PT4: upstream pipe crown 0.30 m before the chamber wall.  For a
            # full cell the crown gauge pressure is the invert pressure minus
            # one diameter of water; for a gassy cell the crown sees the
            # pocket gas pressure (invert pressure minus the local depth).
            h4 = float(min(h_now[i_pt4], case.Du))
            rec["PT4"].append((float(P[i_pt4]) - RHO_L * G * h4) / 1000.0)
            rec["hr"].append(hr)
            rec["S"].append(S)
            rec["Qin"].append(Qin)
...
if __name__ == "__main__":
    # Case C9: Q 25 -> 40 L/s, hr0 = 0.30 m, sealed upstream crown pocket
    case = LiuCase(Q0=0.025, Q1=0.040, t_end=8.0,
                   downstream_full=True, series_c=True, hr0=0.30)
    rec = run_case(case)
    t = np.asarray(rec["t"])
    PT2 = np.asarray(rec["PT2"]); PT3 = np.asarray(rec["PT3"])
    PT4 = np.asarray(rec["PT4"])
    ...
    print(json.dumps(dict(
        ...
        PT4_initial=float(np.mean(PT4[:50])),
        PT2_final=float(np.mean(PT2[-200:])),
        PT3_final=float(np.mean(PT3[-200:])),
        PT4_final=float(np.mean(PT4[-200:])),
        overflow_L=rec["overflow_vol"] * 1e3,
        gas_vented_kg=rec["gas_vented_kg"],
